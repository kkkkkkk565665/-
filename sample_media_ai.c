...
static HI_VOID OpenCameraAiProcess(VIDEO_FRAME_INFO_S frm, VO_LAYER voLayer, VO_CHN voChn)
{
    int ret;
    ...
    SAMPLE_CHECK_EXPR_GOTO(ret < 0, HAND_RELEASE,
        "hand classify plug cal FAIL, ret=%#x\n", ret);
    SAMPLE_PRT("55555555555, weight:%d, height:%d\n", frm.stVFrame.u32Width, frm.stVFrame.u32Height);
    OpencvDemo(&resizeFrm,&frm);
    SAMPLE_PRT("66666666666, weight:%d, height:%d\n", frm.stVFrame.u32Width, frm.stVFrame.u32Height);

    ret = HI_MPI_VO_SendFrame(voLayer, voChn, &frm, 0);
    SAMPLE_PRT("77777777777, weight:%d, height:%d\n", frm.stVFrame.u32Width, frm.stVFrame.u32Height);
    SAMPLE_CHECK_EXPR_GOTO(ret != HI_SUCCESS, HAND_RELEASE,
        "HI_MPI_VO_SendFrame fail, Error(%#x)\n", ret);
...
static HI_VOID* GetVpssChnFrameOpenCamera(HI_VOID* arg)
{
    int ret;
    VIDEO_FRAME_INFO_S frm;
    HI_S32 s32MilliSec = 2000;
    ...
    while (HI_FALSE == g_bAiProcessStopSignal) {
        ...
        SAMPLE_PRT("444444, weight:%d, height:%d\n", frm.stVFrame.u32Width, frm.stVFrame.u32Height);
        SAMPLE_PRT("get vpss frame success, weight:%d, height:%d\n", frm.stVFrame.u32Width, frm.stVFrame.u32Height);
        ...
        SAMPLE_PRT("i am 5");
        OpenCameraAiProcess(frm, voLayer, voChn);
        SAMPLE_PRT("i am 3");
    }
...
static HI_S32 OpenCameraThreadProcess(HI_VOID)
{
    HI_S32 s32Ret;
    if (snprintf_s(acThreadName, BUFFER_SIZE, BUFFER_SIZE - 1, "AIProcess") < 0) {
        HI_ASSERT(0);
    ...
    prctl(PR_SET_NAME, (unsigned long)acThreadName, 0, 0, 0);
    SAMPLE_PRT("i am 3");
    s32Ret = pthread_create(&g_aiProcessThread, NULL, GetVpssChnFrameOpenCamera, NULL);
...
HI_S32 SAMPLE_MEIDA_OPEN(HI_VOID) 
{
    HI_S32             s32Ret;
    HI_S32             fd = 0;
    ...
    /* create work thread to run ai */
    printf("i am 2");
    s32Ret = OpenCameraThreadProcess();
